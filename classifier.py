...
class Classifier(object):
    ...
    def test(self,data,category):
        if self.predict(data) == category:
            return 1
        else:
            return 0
    ...
    def calc_p_w_given_category(self, w):
        p_w_given = {}
        for category in self.categories:
            p_w_given[category] =  (1000)*(self.word_counts[category].get(w, 0.0) + 1) / (sum(self.word_counts[category].values()) + sum(self.vocab.values()))
        return p_w_given
